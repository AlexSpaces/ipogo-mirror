...
:root{color-scheme:dark}*{box-sizing:border-box}body{margin:0;background:#000;color:#fff;font:16px/1.5 -apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}.card{width:min(560px,calc(100% - 40px));margin:12vh auto 60px}h1{margin:0;font-size:1.8rem}p,.updated{color:#aaa}.actions{display:flex;gap:16px;margin:24px 0}.button,.github{color:#fff;text-underline-offset:4px}.button:first-child{font-weight:700}.github{position:fixed;top:20px;right:24px;font-size:.9rem}.updated{margin:28px 0 8px;font-size:.85rem}.versions{margin:0;padding:0;list-style:none;border-top:1px solid #333}.versions li{display:flex;justify-content:space-between;gap:16px;padding:10px 0;border-bottom:1px solid #333}.versions span:last-child,.hash{font-family:ui-monospace,SFMono-Regular,Menlo,monospace}.hash-label{display:block;margin:18px 0 5px;color:#aaa;font-size:.85rem}.hash{overflow-wrap:anywhere;color:#ccc;font-size:.75rem}details{margin-top:16px}summary{cursor:pointer;color:#aaa}.empty{color:#aaa}</style></head>
<body><main class="card"><a class="github" href="https://github.com/AlexSpaces/ipogo-mirror">GitHub</a><h1>iPoGo Mirror</h1><p>A mirror of the iPoGo package repository.</p><div class="actions"><a class="button" href="${sileo}">Add to Sileo</a><a class="button" href="https://ipogo.app/repo">Origin</a></div><p class="updated">Last updated: ${escapeHtml(updated)}</p>${ipogo ? `<ul class="versions"><li><span>${escapeHtml(ipogo.name)}</span><span>${escapeHtml(ipogo.version)}</span></li></ul>${ipogo.sha256 ? `<span class="hash-label">SHA-256</span><div class="hash">${escapeHtml(ipogo.sha256)}</div>` : ""}` : `<p class="empty">Package information is temporarily unavailable.</p>`}<details><summary>Other packages (${remaining.length})</summary><ul class="versions">${remaining.map(({ name, version }) => `<li><span>${escapeHtml(name)}</span><span>${escapeHtml(version)}</span></li>`).join("")}</ul></details></main></body></html>`, {
